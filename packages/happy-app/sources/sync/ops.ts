...
// Route to the correct socket for a machine (multi-server aware)
function getSocketForMachine(machineId: string) {
    const conn = sync.getConnectionForMachine(machineId);
    return conn || apiSocket;
}

// Route to the correct socket for a session (multi-server aware)
function getSocketForSession(sessionId: string) {
    const conn = sync.getConnectionForSession(sessionId);
    return conn || apiSocket;
}
...
export async function machineSpawnNewSession(options: SpawnSessionOptions): Promise<SpawnSessionResult> {
    ...
    try {
        const result = await getSocketForMachine(machineId).machineRPC<SpawnSessionResult, {
...
export async function machineStopDaemon(machineId: string): Promise<{ message: string }> {
    const result = await getSocketForMachine(machineId).machineRPC<{ message: string }, {}>(
        machineId,
        'stop-daemon',
        {}
    );
...
export async function machineBash(
    ...
    try {
        const result = await getSocketForMachine(machineId).machineRPC<{
            success: boolean;
            stdout: string;
            stderr: string;
            exitCode: number;
...
export async function machineUpdateMetadata(
    ...
    while (retryCount < maxRetries) {
        ...
        const result = await getSocketForMachine(machineId).emitWithAck<{
...
export async function sessionAbort(sessionId: string): Promise<void> {
    await getSocketForSession(sessionId).sessionRPC(sessionId, 'abort', {
...
export async function sessionAllow(sessionId: string, id: string, mode?: 'default' | 'acceptEdits' | 'bypassPermissions' | 'plan', allowedTools?: string[], decision?: 'approved' | 'approved_for_session'): Promise<void> {
    ...
    await getSocketForSession(sessionId).sessionRPC(sessionId, 'permission', request);
}
...
export async function sessionSwitch(sessionId: string, to: 'remote' | 'local'): Promise<boolean> {
    ...
    const response = await getSocketForSession(sessionId).sessionRPC<boolean, SessionModeChangeRequest>(
        sessionId,
        'switch',
        request,
    );
...
export async function sessionBash(sessionId: string, request: SessionBashRequest): Promise<SessionBashResponse> {
    try {
        const response = await getSocketForSession(sessionId).sessionRPC<SessionBashResponse, SessionBashRequest>(
            sessionId,
            'bash',
            request
        );
...
export async function sessionReadFile(sessionId: string, path: string): Promise<SessionReadFileResponse> {
    try {
        ...
        const response = await getSocketForSession(sessionId).sessionRPC<SessionReadFileResponse, SessionReadFileRequest>(
            sessionId,
            'readFile',
            request
        );
...
export async function sessionWriteFile(
    ...
    try {
        ...
        const response = await getSocketForSession(sessionId).sessionRPC<SessionWriteFileResponse, SessionWriteFileRequest>(
            sessionId,
            'writeFile',
            request
        );
...
export async function sessionListDirectory(sessionId: string, path: string): Promise<SessionListDirectoryResponse> {
    try {
        ...
        const response = await getSocketForSession(sessionId).sessionRPC<SessionListDirectoryResponse, SessionListDirectoryRequest>(
            sessionId,
            'listDirectory',
            request
        );
...
export async function sessionGetDirectoryTree(
    ...
    try {
        ...
        const response = await getSocketForSession(sessionId).sessionRPC<SessionGetDirectoryTreeResponse, SessionGetDirectoryTreeRequest>(
            sessionId,
            'getDirectoryTree',
            request
        );
...
export async function sessionRipgrep(
    ...
    try {
        ...
        const response = await getSocketForSession(sessionId).sessionRPC<SessionRipgrepResponse, SessionRipgrepRequest>(
            sessionId,
            'ripgrep',
            request
        );
...
export async function sessionKill(sessionId: string): Promise<SessionKillResponse> {
    try {
        const response = await getSocketForSession(sessionId).sessionRPC<SessionKillResponse, {}>(
            sessionId,
            'killSession',
            {}
        );
...
export async function sessionDelete(sessionId: string): Promise<{ success: boolean; message?: string }> {
    try {
        ...
        const response = await socket.request(`/v1/sessions/${sessionId}`, {
            method: 'DELETE'
        });
